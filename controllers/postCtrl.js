...
const postCtrl = {
  ...
  getAllLikesByPostId: async (req, res) => {
    try {
      ...
      if (!likes)
        return res.status(400).json("Likes in this post does not exist");
      ...
      if (check) {
        if (check.type === "like") {
          if (likeType === "like") {
            return res
              .status(400)
              .json({ msg: "You can add 1 like/dislike only" });
  ...
  updatePostById: async (req, res) => {
    ...
    if (post.author !== req.user.id)
      return res
        .status(401)
        .json({ msg: "Only the author can update the post" });
  ...
  deletePostById: async (req, res) => {
    try {
      ...
      if (post.author === req.user.id || req.user.role === "admin") {
        ...
        return res
          .status(401)
          .json({ msg: "Only the author or admin can delete the post" });
    } catch (err) {
      return res.status(500).json({ msg: err.message });
    }
  },
